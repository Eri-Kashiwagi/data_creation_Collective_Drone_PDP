...
# 返回满足能耗预算 B 的最小 y（1/2/3）。
def min_feasible_y_energy(
...
# 极坐标转换为平面坐标（以 depot 为中心）。
def polar_to_xy(
    depot: Tuple[float, float], r: float, theta: float
) -> Tuple[float, float]:
...
def gen_time_windows(
    rng: random.Random, d0p: float, dpd: float, dd0: float, tw: TWCfg
    ...
    e_p = max(0, min(tw.horizon, e_p))
    l_p = max(0, min(tw.horizon, l_p))
    e_d = max(0, min(tw.horizon, e_d))
    l_d = max(0, min(tw.horizon, l_d))
    if l_p < e_p:
        l_p = e_p
    if l_d < e_d:
        l_d = e_d
...
def generate_one_order(
    ...
    for _ in range(max_tries):
        ...
        r_lo, r_hi, dbg = ring_interval(
            alpha, w_base, B, q, pd_ratio, domain_max, target_y
        )
        ...
        # pickup 的角度取决于空间分布模式。
        theta_p = sample_theta(
            spatial.pattern, rng, theta_centers, spatial.sigma_theta, spatial.p_cluster
        )
    ...
    # 连续多次失败则提示调整参数。
    raise RuntimeError(
        "Failed to generate order. Try --auto_grid or adjust B/pd_ratio."
    )
...
def write_txt(
    ...
    # depot 行。
    lines.append(
        f"0\t{int(round(depot[0]))}\t{int(round(depot[1]))}\t0\t0\t{horizon}\t0\t0\t0"
    )
    ...
    for i, od in enumerate(orders, start=1):
        ...
        d_id = n + i
        lines.append(
            f"{i}\t{int(round(px))}\t{int(round(py))}\t{q}\t{e}\t{l}\t{service}\t0\t{d_id}"
        )
    ...
    for i, od in enumerate(orders, start=1):
        ...
        d_id = n + i
        lines.append(
            f"{d_id}\t{int(round(dx))}\t{int(round(dy))}\t{-q}\t{e}\t{l}\t{service}\t{p_id}\t0"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        type=float,
        default=1.15,
        help="bigger => harder energy => more y=2/3",
    )
...
# 根据能耗环自动扩展网格尺寸。
def auto_grid(
    args: argparse.Namespace, alpha: float, w_base: float, B: float, single_Q: float
) -> float:
...
def main() -> None:
    ...
    alpha = calc_alpha(
        args.gravity, args.air_density, args.blade_area, args.rotor_height
    )
    ...
    for scale_name, n_orders in scales:
        for cls_name, spatial, tw in classes:
            ...
            for k in range(1, args.K + 1):
                inst_seed = (
                    args.seed + 100000 * k + (hash((scale_name, cls_name)) % 10007)
                )
                ...
                with open(
                    txt_path.replace(".txt", ".json"), "w", encoding="utf-8"
                ) as f:
